...
def runs_attacker(t: str, same_type: bool, combs, df):
    strong, weak, normal = attack_side(t, combs, same_type, df)
...
def runs_defender(types, same_type: bool, all_types, df):
    strong, weak, normal = defend_side(types, [t for (t,) in all_types], same_type, df)
    print('克制：')
    for res in strong:
        print(to_string(res))
...
if __name__ == "__main__":
    ...
    for _, value in parser.parse_args()._get_kwargs():
        if len(value) == 1:
            defender_types = (value[0],)
        elif len(value) == 2:
            defender_types = tuple(value)
